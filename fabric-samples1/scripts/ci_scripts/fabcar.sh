...
logs() {
    LOG_DIRECTORY=$WORKSPACE/CardioGene/$1
...
cd $WORKSPACE/$BASE_DIR/CardioGene || exit
export PATH=gopath/src/github.com/hyperledger/fabric-samples1/bin:$PATH

LANGUAGES="go java javascript typescript"
for LANGUAGE in ${LANGUAGES}; do
    echo -e "\033[1m ${LANGUAGE} Test\033[0m"
    echo -e "\033[32m starting CardioGene test (${LANGUAGE})" "\033[0m"
    # Start Fabric, and deploy the smart contract
    ./startFabric.sh ${LANGUAGE}
    copy_logs $? CardioGene-${LANGUAGE}
    ...
    if [ -d ${LANGUAGE} ]; then
        ...
        if [ ${LANGUAGE} = "javascript" -o ${LANGUAGE} = "typescript" ]; then
            ...
            copy_logs $? CardioGene-${LANGUAGE}-enrollAdmin
            ${COMMAND} ${PREFIX}registerUser${SUFFIX}
            copy_logs $? CardioGene-${LANGUAGE}-registerUser
            ${COMMAND} ${PREFIX}query${SUFFIX}
            copy_logs $? CardioGene-${LANGUAGE}-query
            ${COMMAND} ${PREFIX}invoke${SUFFIX}
            copy_logs $? CardioGene-${LANGUAGE}-invoke
    ...
    echo -e "\033[32m finished CardioGene test (${LANGUAGE})" "\033[0m"
done
