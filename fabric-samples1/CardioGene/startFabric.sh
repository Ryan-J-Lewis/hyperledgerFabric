...
if [ "$CC_SRC_LANGUAGE" = "go" -o "$CC_SRC_LANGUAGE" = "golang"  ]; then
	...
	CC_SRC_PATH=github.com/hyperledger/fabric-samples1/chaincode/CardioGene/go
	...
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples1/chaincode/CardioGene/java/build/install/CardioGene
  ...
  echo Finished compiling Java code
	...
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples1/chaincode/CardioGene/javascript
elif [ "$CC_SRC_LANGUAGE" = "typescript" ]; then
	CC_RUNTIME_LANGUAGE=node # chaincode runtime language is node.js
	CC_SRC_PATH=/opt/gopath/src/github.com/hyperledger/fabric-samples1/chaincode/CardioGene/typescript
	echo Compiling TypeScript code into JavaScript ...
	pushd ../chaincode/CardioGene/typescript
	npm install
	npm run build
	popd
...
REGEX='Package ID: (.*), Label: CardioGeneV1'
if [[ `${PEER0_ORG1} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  PACKAGE_ID_ORG1=${BASH_REMATCH[1]}
else
  echo Could not find package ID for CardioGeneV1 chaincode on peer0.org1.example.com
  exit 1
fi
...
${PEER0_ORG2} lifecycle chaincode install CardioGene.tar.gz

echo "Installing smart contract on peer1.org2.example.com"
${PEER1_ORG2} lifecycle chaincode install CardioGene.tar.gz
...
if [[ `${PEER0_ORG2} lifecycle chaincode queryinstalled` =~ $REGEX ]]; then
  ...
  echo Could not find package ID for CardioGeneV1 chaincode on peer0.org2.example.com
  exit 1
fi
...
Next, use the CardioGene applications to interact with the deployed CardioGene contract.
